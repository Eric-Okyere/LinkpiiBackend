...
router.get(`/`, async (req,res)=>{
 ...
 router.get(`/approved`, async (req, res) => {
  try {
    const approvedProducts = await Product.find({ approved: true }).populate("category").populate("author").sort({boost:-1, dateCreated: -1 });
...
router.post('/', upload.fields([
  ...
  router.put('/:id', upload.fields([
    ...
    try {
      // Extract data from the request
      const { name, description, region, town, phone, location, price, category,whatsapp, condition } = req.body;
