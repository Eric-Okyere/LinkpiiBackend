...
const { Equipments } = require('../models/categories/Equipments');
...
router.get(`/`, async (req,res)=>{
    ...
    const productList = await Equipments.find()
    if(!productList){
 ...
 router.get(`/:id`, async (req,res)=>{
    const product = await Equipments.findById(req.params.id)
    if(!product){
        res.status(500).json({success: false})
    }
    res.send(product)
...
router.post('/', upload.single('picture'), async (req, res) => {
  try {
    ...
    const newProduct = new Equipments({
      name,
 ...
 router.delete("/:id",(req, res)=>{
    Equipments.findByIdAndRemove(req.params.id).then(user=>{
        if(user){
            return res.status(200).json({success:true, message:"the equipment is deleted successfully"})
        } else{
