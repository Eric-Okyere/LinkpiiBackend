...
router.get(`/`, async (req,res)=>{
    ...
    const productList = await Spare.find().sort({ dateCreated: -1 })
    if(!productList){
...
// 
 ...
 router.get(`/approve/approved`, async (req, res) => {
  try {
    const approvedProducts = await Spare.find({ approved: true }).sort({boost:-1, dateCreated: -1 });
 ...
 router.get('/user/:id',async(req, res)=>{
    ...
    const userItems=await Spare.find({author:req.params.id}).sort({ dateCreated: -1 })
...
router.get('/get/count', async (req, res) => {
  try {
    const productCount = await Spare.countDocuments();
...
router.put('/:id/approve', async (req, res) => {
  ...
  try {
    const product = await Spare.findByIdAndUpdate(productId, { approved: true }, { new: true });
...
router.put('/:id/boost', async (req, res) => {
  ...
  try {
    ...
    const allApprovedProducts = await Spare.find({ approved: true }).sort({ boost: -1, dateCreated: -1 });
...
router.put('/:id/deactivate', async (req, res) => {
  ...
  try {
    const product = await Spare.findByIdAndUpdate(productId, { approved: false }, { new: true });
 ...
 router.delete("/:id",(req, res)=>{
    Spare.findByIdAndRemove(req.params.id).then(product=>{
        if(product){
