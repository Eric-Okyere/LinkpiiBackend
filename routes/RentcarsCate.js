...
const { Rentcarscats } = require('../models/categories/RentCars');
...
router.get(`/`, async (req,res)=>{
    ...
    const productList = await Rentcarscats.find()
    if(!productList){
 ...
 router.get(`/:id`, async (req,res)=>{
    const product = await Rentcarscats.findById(req.params.id)
    if(!product){
        res.status(500).json({success: false})
    }
    res.send(product)
...
router.post('/', upload.single('picture'), async (req, res) => {
  try {
    ...
    const newProduct = new Rentcarscats({
      name,
 ...
 router.delete("/:id",(req, res)=>{
    Rentcarscats.findByIdAndRemove(req.params.id).then(user=>{
        if(user){
