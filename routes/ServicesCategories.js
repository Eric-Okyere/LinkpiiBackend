...
const { Servicescat } = require('../models/categories/Services');
...
router.get(`/`, async (req,res)=>{
    ...
    const productList = await Servicescat.find()
    if(!productList){
 ...
 router.get(`/:id`, async (req,res)=>{
    const product = await Servicescat.findById(req.params.id)
    if(!product){
        res.status(500).json({success: false})
    }
    res.send(product)
...
router.post('/', upload.single('picture'), async (req, res) => {
  try {
    ...
    const newProduct = new Servicescat({
      name,
 ...
 router.delete("/:id",(req, res)=>{
    Servicescat.findByIdAndRemove(req.params.id).then(user=>{
        if(user){
